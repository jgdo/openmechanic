...
RigidBodyPartVisualizer::RigidBodyPartVisualizer(GameVisualization* gameViz) :
...
void RigidBodyPartVisualizer::init(const AsyncObjectContainer::DataReadIndex< RigidBodyPartData >& idx, OID parentFullBodyID)
{
	// TODO check if called twice
	_bodyPartID = idx.bin.rID;
	_parentFullBodyID = parentFullBodyID;
	
	_transform = new osg::MatrixTransform;
	_gameViz->rootNode->addChild(_transform.get());
}

void RigidBodyPartVisualizer::changed(const AsyncObjectContainer::DataReadIndex< RigidBodyPartData >& idx)
{
	_transform->setMatrix(osgbCollision::asOsgMatrix(idx->worldTransform));
}


void RigidBodyPartVisualizer::blockAdded(const AsyncObjectContainer::DataReadIndex< RigidBodyPartData >& idx, 
										 const BodyBlock* block, BlockIndex index, btQuaternion relRot, OID attachedControlOID)
{
	...
	auto entry = BlockEntryPtr(new BlockEntry(this, blockNode, block, index, relRot, attachedControlOID));
...
RigidBodyPartVisualizer::BlockEntry::BlockEntry(RigidBodyPartVisualizer* parent, osg::Geode* geode, const BodyBlock* bodyBlock, BlockIndex const& centerIndex, const btQuaternion& relRot, OID controlRID):
...
void RigidBodyPartVisualizer::blockRemoved(const AsyncObjectContainer::DataReadIndex< RigidBodyPartData >& idx, BlockIndex index)
{
